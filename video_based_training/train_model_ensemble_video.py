"""Makes the training run for the ensemble model"""
import pandas as pd
import torch
...
from make_dataset_dataloader_video import (
    get_device,
    shift_to_device,
    make_train_val_dataset,
)
from make_dataset_dataloader_video import GPUDataLoader
from create_model_pretrained import ResNetModel

MODALITY = "VideoEnsemble"
DATA_PATH = [
    "modalities_output/albedo",
    "modalities_output/normal_map",
    "modalities_output/shadow",
    "modalities_output/shading",
    "modalities_output/lighting",
    "deep_fake_modality_generation/frames_data",
]
...
## Optimizer and loss ##
optim = SGD(model.parameters(), lr=0.006, weight_decay=1e-5)
criterion = nn.CrossEntropyLoss()

## Making datasets ##
train_dataset, val_dataset = make_train_val_dataset(
    DATA_PATH, split_ratio=0.8, ensemble=True
)

## Creating dataloaders ##
train_dataloader = DataLoader(
    train_dataset, batch_size=64, shuffle=True, num_workers=8, prefetch_factor=10
)
val_dataloader = DataLoader(
    val_dataset, batch_size=64, shuffle=True, num_workers=8, prefetch_factor=10
)
