...
parser.add_argument("--lighting", type=bool , default=False , help = "Sets if lighting modality is needed")
parser.add_argument("--shading", type=bool , default=False , help = "Sets if shading modality is needed")
parser.add_argument("--shadow", type=bool , default=False , help = "Sets if shadow modality is needed")
parser.add_argument("--albedo", type=bool , default=False , help = "Sets if albedo modality is needed")
parser.add_argument("--normal_map", type=bool , default=False , help = "Sets if normal map modality is needed")

parser.add_argument("--lighting_name", type=str, default=None, help="Name of the lighting image")
parser.add_argument("--shading_name", type=str, default=None, help="Name of the lighting image")
parser.add_argument("--shadow_name", type=str, default=None, help="Name of the lighting image")
parser.add_argument("--albedo_name", type=str, default=None, help="Name of the lighting image")
parser.add_argument("--normal_map_name", type=str, default=None, help="Name of the lighting image")
...
def saving_result(results, dst_dir, lighting_flag , lighting_name , albedo_flag ,
                  albedo_name , normal_map_flag , normal_map_name , shadow_flag ,
                  shadow_name , shading_flag , shading_name , prefix=""):
    ...
    if normal_map_flag:
        nm_pred_path = os.path.join(dst_dir, prefix + normal_map_name)
        io.imsave(nm_pred_path, nm_pred)
    if albedo_flag:
        albedo_path = os.path.join(dst_dir, prefix + albedo_name)
        io.imsave(albedo_path, albedos)
    if shading_flag:
        shading_path = os.path.join(dst_dir, prefix + shading_name)
        io.imsave(shading_path, shading)
    if shadow_flag:
        shadow_path = os.path.join(dst_dir, prefix + shadow_name)
        io.imsave(shadow_path, shadow)
    if lighting_flag:
        lighting_path = os.path.join(dst_dir, lighting_name)
...
if args.mode == "demo_im":
    ...
    lighting_name = args.lighting_name
    albedo_flag = args.albedo
    albedo_name = args.albedo_name
    normal_map_flag = args.normal_map
    normal_map_name = args.normal_map_name
    shadow_flag = args.shadow
    shadow_name = args.shadow_name
    shading_flag = args.shading
    shading_name = args.shading_name
    ...
    saving_result(results, dst_dir , lighting_flag , lighting_name , albedo_flag ,
                  albedo_name , normal_map_flag , normal_map_name , shadow_flag ,
                  shadow_name , shading_flag , shading_name)
